  ...
  <script>
    ...
    function renderResult(data) {
      ...
      if (data.verified) {
        ...
        container.innerHTML = `<div class="result"><div class="result-banner success"><span class="icon">✓</span><div class="meta"><span>Identity Verified</span><small>${data.steps.length} steps</small></div></div><div class="steps-title">Proof Derivation</div><div class="proof-container">${stepsHtml}</div></div>`;
      } else {
        const msg = (data.error || 'The identity could not be proven within search limits.').replace(/</g, '&lt;');
        container.innerHTML = `<div class="result"><div class="result-banner failure"><span class="icon">✗</span><div class="meta"><span>Could Not Verify</span><small>${msg}</small></div></div></div>`;
      }
    }
